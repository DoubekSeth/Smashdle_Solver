...
```{r}
# Data Sheet
smashdle_csv = Smashdle.CSV...Sheet1
View(smashdle_csv)
```
...
```{r}
# Checks if a character matches a given state pattern
check_state = function(g, smasher, state, characters){
  for(i in 1:ncol(state)){
    ...
    if(state[i] == "Incorrect"){
      ...
      g_part_vec = strsplit(g_cat_val, ", ")[[1]]
      smasher_part_vec = strsplit(smasher_cat_val, ", ")[[1]]
...
```{r}
entropy("Mr. Game & Watch", possible_state_combos, smashdle_csv)
```
