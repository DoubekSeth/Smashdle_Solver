...
expand_children = function(graph){
  for(i in V(graph)){
    ...
    if(degree(graph, V(graph)[i]) <= 1 && length(V(graph)[[i]]$roster) > 1){
      ...
      j = 1
      for(i in (old_vertex_count+1):new_vertex_count){
        #Add new roster
        print(next_state_roster_rosters[[j]])
        V(graph)[[i]]$roster = next_state_roster_rosters[[j]]
        print(V(graph)[[i]]$roster)
        # Add new label (best guess)
        # Now, find the best character
        remaining_roster = V(graph)[[i]]$roster
        if(length(remaining_roster) == 1){
          V(graph)$label[[i]] = remaining_roster[1]
          ...
          V(graph)$label[[i]] = best_char_name
...
# for (i in 2:starting_vertices_count){
#   #get the roster
#   remaining_roster = V(flowchart)$roster[[i]]
#   
#   #Now, find the best character
#   if(length(remaining_roster) == 1){
#     V(flowchart)$label[[i]] = remaining_roster[1]
#     print(remaining_roster)
#   } else {
#     remaining_roster_df = smashdle_csv[smashdle_csv$Character %in% remaining_roster, ]
#     best_char_name = best_char(possible_state_combos, remaining_roster_df)
#     V(flowchart)$label[[i]] = best_char_name
#     print(best_char_name)
#     print(remaining_roster)
#   }
# }

#Adding the states to the edges
#E(flowchart)$state = first_guess_combos[[2]]
plot(flowchart, layout = layout_nicely(flowchart), asp = .25, vertex.size = 10)
```
